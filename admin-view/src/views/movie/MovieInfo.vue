...
<!--      操作按钮-->
    ...
    <el-dialog title="添加电影" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px">
        ...
        <el-form-item label="电影区域" prop="movieAreaId">
          <el-select v-model="addForm.movieAreaId" placeholder="请选择电影地区" clearable>
            <el-option
                v-for="item in movieAreaList"
                :key="item.movieAreaId"
                :label="item.movieAreaName"
        ...
        <el-form-item label="电影年代" prop="movieAgeId">
           <el-select v-model="addForm.movieAgeId" placeholder="请选择电影年代" clearable>
            <el-option
                v-for="item in movieAgeList"
                :key="item.movieAgeId"
                :label="item.movieAgeName"
    ...
    <el-dialog title="修改电影信息" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="电影区域" prop="movieAreaId">
            <el-select v-model="editForm.movieAreaId" placeholder="请选择电影地区" clearable>
            <el-option
                v-for="item in movieAreaList"
                :key="item.movieAreaId"
                :label="item.movieAreaName"
        ...
        <el-form-item label="电影年代" prop="movieAgeId">
             <el-select v-model="editForm.movieAgeId" placeholder="请选择电影年代" clearable>
            <el-option
                v-for="item in movieAgeList"
                :key="item.movieAgeId"
                :label="item.movieAgeName"
...
export default {
  ...
  methods: {
    getMovieList() {
      ...
      const _this = this;
      axios.get('/Movie/sysMovie', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    getCategoryList() {
      const _this = this
      axios.get('/Movie/sysMovieCategory').then(resp=>{
    ...
    getMovieAreaList() {
      const _this = this;
      axios.get('/Movie/sysMovieArea').then(resp => {
        console.log(resp)
    ...
    getMovieAgeList() {
      const _this = this;
      axios.get('/Movie/sysMovieAge').then(resp => {
        console.log(resp)
    ...
    getactorList(){
      const _this = this;
      axios.get('/Actor/sysActor').then(resp =>{
    ...
    getroleList(){
      const _this = this;
      axios.get('/Actor/sysActorRole').then(resp =>{
        console.log(resp.data.data)
    ...
    async addMovie() {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post('/Movie/sysMovie', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    async editHallInfo() {
      ...
      this.$refs.editFormRef.validate(async valid => {
        ...
        let success = true
        axios.defaults.headers.put['Content-Type'] = 'application/json'
        await axios.put('/Movie/sysMovie', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200) {
    ...
    async showEditDialog(id1) {
      const _this = this
      await axios.get('/Movie/sysMovie/' + id1).then(resp => {
        console.log(resp)
    ...
    async showEditActorDialog(id1){
      ...
      await axios.get('/Movie/sysMovie/find/'+id1).then(response=>{
        console.log('电影演员列表')
    ...
    async multipleDelete() {
      ...
      console.log(ids)
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      await axios.delete('/Movie/sysMovie/' + ids, JSON.stringify(ids)).then(resp => {
        console.log(123)
        if (resp.data.code !== 200) {
    ...
    async deleteMovieById(id1) {
      ...
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      await axios.delete('/Movie/sysMovie/' + deleteInfo, JSON.stringify(deleteInfo)).then(resp => {
        if (resp.data.code !== 200) {
    ...
    async deleteActor(rid,aid){
      ...
      const _this = this
      console.log(111)
      await axios.delete('/Actor/sysActorMovie/'+_this.actorMovieId+'/'+aid+'/'+rid).then(resp =>{
        if(resp.data.code!== 200){
    ...
    async addActor(){
      ...
      const _this = this
        let success = true
      await axios.post('/Actor/sysActorMovie',obj).then(resp=>{
        if(resp.data.code!== 200){
        ...
        if(!success) return
        await axios.get('/Movie/sysMovie/find/'+_this.actorMovieId).then(response=>{
          console.log('电影演员列表')
          console.log(response.data.data)
          _this.editActorForm = response.data.data.actorRoleList
        })
    ...
    async deleteCategory(categoryId){
      ...
      const _this = this
      await axios.delete('/Movie/sysMovieToCategory/'+this.actorMovieId+'/'+categoryId).then(resp=>{
        console.log(resp)
        _this.$message.success('删除类型成功')
      })
      await axios.get('/Movie/sysMovie/find/'+this.actorMovieId).then(response=>{
        _this.editCategoryForm = response.data.data.movieCategoryList
      })
    },
    handleExceed(){
    ...
    async addCategory(){
      const _this = this
      await axios.post('/Movie/sysMovieToCategory/'+this.actorMovieId+'/'+this.selectedMovieCategory).then(resp=>{
        console.log(resp)

      })
      axios.get('/Movie/sysMovie/find/'+this.actorMovieId).then(response=>{
        _this.editCategoryForm = response.data.data.movieCategoryList
      })
    }
  }
