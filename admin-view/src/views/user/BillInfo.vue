      ...
      <el-table :data="billList" style="width: 100%" border stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="billState" label="订单状态">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.billState" disabled="true" @change="billStateChanged(scope.row)"></el-switch>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getBillList() {
      ...
      const _this = this;
      axios.get('/Bill/sysBill', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    async multipleDelete(){
      ...
      await axios.delete('/Bill/sysBill/' + ids).then(resp => {
    ...
    async billStateChanged(billInfo){
      ...
      await axios.put('/Bill/sysBill', JSON.stringify(billInfo)).then(resp => {
        if (resp.data.code !== 200){
