...
export default {
  ...
  methods: {
    getCinemaList() {
      ...
      const _this = this;
      axios.get('/Cinema/sysCinema', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    async addCinema(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post('/Cinema/sysCinema', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
          if (resp.data.code !== 200){
            this.$message.error('添加影院信息失败！信息重复')
          }else{
    ...
    async showEditDialog(id){
      const _this = this
      await axios.get('/Cinema/sysCinema/' + id).then(resp => {
        console.log(resp)
    ...
    async editCinemaInfo(){
      ...
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('/Cinema/sysCinema', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200){
    ...
    async multipleDelete(){
      ...
      console.log(ids)
      await axios.delete('/Cinema/sysCinema/' + ids).then(resp => {
        if (resp.data.code !== 200){
    ...
    async deleteCinemaById(id){
      ...
      await axios.delete('/Cinema/sysCinema/' + id).then(resp => {
        if (resp.data.code !== 200){
    ...
    getCinemaBrandList(){
      const _this = this
      axios.get('/Cinema/sysCinemaBrand').then(resp => {
        console.log(resp)
    ...
    getCinemaAreaList(){
      const _this = this
      axios.get('/Cinema/sysCinemaArea').then(resp => {
        console.log(resp)
    ...
    async stateChanged(cinemaInfo){
      ...
      const _this = this
      axios.defaults.headers.put['Content-Type'] = 'application/json'
      await axios.put('/Cinema/sysCinema', JSON.stringify(cinemaInfo)).then(resp => {
        if (resp.data.code !== 200){
