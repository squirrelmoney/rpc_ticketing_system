  ...
  methods: {
    ...
    addSession(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.get('/Hall/sysHall/' + this.addForm.cinemaId + '/' + this.addForm.hallId).then(resp => {
        ...
        await axios.post('sysSession', JSON.stringify(_this.addForm)).then(resp => {
          ...
          if (resp.data.code !== 200){
            this.$message.error('添加场次信息失败！场次冲突')
          }else{
    ...
    editSessionInfo(){
      ...
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('/sysSession', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200){
            this.$message.error('修改场次信息失败！场次冲突')
            success = false
          }else{
    ...
    getHallList(){
      const _this = this
      axios.get('/Hall/sysHall?cinemaId=' + this.loginUser.cinemaId).then(resp => {
        _this.hallList = resp.data.data;
      })
    },
    getMovieList(){
      const _this = this
      axios.get('/Movie/sysMovie').then(resp => {
        _this.movieList = resp.data.data;
      })
    },
    getMovieRuntimeList(){
      const _this = this
      axios.get('/Movie/sysMovieRuntime').then(resp => {
        console.log(resp)
