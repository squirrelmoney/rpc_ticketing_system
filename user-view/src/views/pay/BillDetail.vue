    ...
    <div class="bill-info-container">
      ...
      <div class="bill-info-desc">
        <div class="bill-info-header">
          <span>喵喵订单号: {{billInfo.billId}}</span>
...
export default {
  ...
  methods:{
    async getBillInfo(){
      const { data : res } = await axios.get('/Bill/sysBill/' + this.billId)
    ...
    async payForBill(){
      ...
      const { data: res } = await axios.put('/Bill/sysBill',JSON.stringify({sysBill: this.billInfo, sessionSeats: JSON.stringify(sessionSeats)}))
