...
export default {
  ...
  methods: {
    getMovieCommentList() {
      const _this = this;
      axios.get('/Movie/sysMovieComment', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    async getMovieList(){
      const { data : res } = await axios.get('/Movie/sysMovie')
    ...
    addMovieComment(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        const { data : res } = await axios.post('/Movie/sysMovieComment', JSON.stringify(_this.addForm))
    ...
    async multipleDelete(){
      ...
      await axios.delete('/Movie/sysMovieComment/' + ids).then(resp => {
    ...
    async deleteMovieComment(id1,id2,time,c,s){
      ...
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      await axios.post('/Movie/sysMovieComment/delete', JSON.stringify(deleteInfo)).then(resp => {
        console.log(resp);
        console.log(deleteInfo)
        if (resp.data.code !== 200){
