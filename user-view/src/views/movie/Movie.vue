...
export default {
  ...
  methods:{
    async getCategoryList(){
      const { data : res } = await axios.get('/Movie/sysMovieCategory')
      if(res.code !== 200) return this.$message.error('获取服务器信息失败')
      this.categoryList = res.data
    },
    async getAreaList(){
      const { data : res } = await axios.get('/Movie/sysMovieArea')
      if(res.code !== 200) return this.$message.error('获取服务器信息失败')
      this.areaList = res.data
    },
    async getAgeList(){
      const { data : res } = await axios.get('/Movie/sysMovieAge')
      if(res.code !== 200) return this.$message.error('获取服务器信息失败')
