  ...
  <div>
    ...
    <el-dialog title="添加影厅" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px">
        ...
        <el-form-item label="影厅分类" prop="hallCategoryId">
           <el-select v-model="addForm.hallCategoryId" placeholder="请选择影厅分类" clearable>
            <el-option
                v-for="item in hallCategoryList"
                :key="item.hallCategoryId"
                :label="item.hallCategoryName"
    ...
    <el-dialog title="修改影厅" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="影厅分类" prop="hallCategoryId">
           <el-select v-model="editForm.hallCategoryId" placeholder="请选择影厅分类" clearable>
            <el-option
                v-for="item in hallCategoryList"
                :key="item.hallCategoryId"
                :label="item.hallCategoryName"
...
export default {
  ...
  methods: {
    getHallList() {
      ...
      const _this = this;
      axios.get('/Hall/sysHall', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addHall(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        const {data : res } = await axios.post('/Hall/sysHall', JSON.stringify(_this.addForm))
    ...
    showEditDialog(id1, id2){
      const _this = this
      axios.get('/Hall/sysHall/' + id1 + '/' + id2 ).then(resp => {
        console.log(resp)
    ...
    editHallInfo(){
      ...
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('/Hall/sysHall', JSON.stringify(_this.editForm)).then(resp => {
    ...
    async multipleDelete(){
      ...
      console.log(ids)
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      await axios.post('/Hall/sysHall/delete', JSON.stringify(ids)).then(resp => {
        if (resp.data.code !== 200){
    ...
    async deleteHallById(id1, id2){
      ...
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      await axios.post('/Hall/sysHall/delete', JSON.stringify(deleteInfo)).then(resp => {
        if (resp.data.code !== 200){
    ...
    getCinemaList(){
      const _this = this;
      axios.get('/Cinema/sysCinema').then(resp => {
        console.log(resp)
    ...
    getHallCategoryList(){
      const _this = this;
      axios.get('/Hall/sysHallCategory').then(resp => {
        console.log(resp)
    ...
    async hallStateChanged(hallInfo){
      const _this = this
      axios.defaults.headers.put['Content-Type'] = 'application/json'
      await axios.put('/Hall/sysHall', JSON.stringify(hallInfo)).then(resp => {
        if (resp.data.code !== 200){
    ...
    async arrangeSeat(id1, id2){
      const _this = this
      await axios.get('/Hall/sysHall/' + id1 + '/' + id2).then(resp => {
    ...
    async saveSeat(){
      ...
      const _this = this
      axios.defaults.headers.put['Content-Type'] = 'application/json'
      await axios.put('/Hall/sysHall', JSON.stringify(_this.editSeat)).then(resp => {
        if (resp.data.code !== 200){
