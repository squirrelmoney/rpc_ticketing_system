...
export default {
  ...
  methods: {
    getCinemaBrandList() {
      const _this = this;
      axios.get('/Cinema/sysCinemaBrand', {params: _this.queryInfo}).then(resp => {
        console.log(resp)
    ...
    addCinemaBrand(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        await axios.post('/Cinema/sysCinemaBrand', JSON.stringify(_this.addForm)).then(resp => {
          console.log(resp)
    ...
    showEditDialog(id){
      const _this = this
      axios.get('/Cinema/sysCinemaBrand/' + id ).then(resp => {
        console.log(resp)
    ...
    editCinemaBrandInfo(){
      this.$refs.editFormRef.validate(async valid => {
        ...
        await axios.put('/Cinema/sysCinemaBrand', JSON.stringify(_this.editForm)).then(resp => {
          if (resp.data.code !== 200){
    ...
    async multipleDelete(){
      ...
      await axios.delete('/Cinema/sysCinemaBrand/' + ids).then(resp => {
        if (resp.data.code !== 200){
    ...
    async deleteCinemaBrandById(id){
      ...
      await axios.delete('/Cinema/sysCinemaBrand/' + id).then(resp => {
        if (resp.data.code !== 200){
