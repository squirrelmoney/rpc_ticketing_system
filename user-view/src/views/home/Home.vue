...
export default {
  ...
  methods:{
    async getOngoingMovieList(){
      const { data : res } = await axios.get('/Movie/sysMovie', {params: this.queryInfo1})
      this.ongoingMovieList = res.data
      this.total = res.total
    },
    async getUpcomingMovieList() {
      const {data: res} = await axios.get('/Movie/sysMovie', {params: this.queryInfo2})
      this.upcomingMovieList = res.data
      this.total = res.total
    },
    async getClassicMovieList(){
      const { data : res } = await axios.get('/Movie/sysMovie', {params: this.queryInfo3})
    ...
    async getTop100List(){
      const {data: resp} = await axios.get('/Movie/sysMovie/rankingList/4', {params: this.queryInfo4})
      console.log(resp)
      if(resp.code != 200) return this.$message.error(resp.msg)
      this.top100List = resp.data
    },
    async getActorList(){
      const {data: resp} = await axios.get('/Actor/sysActor', {params: this.queryInfo4})
      console.log(resp)
      if(resp.code != 200) return this.$message.error(resp.msg)
